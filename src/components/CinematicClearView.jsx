...
const CinematicClearView = ({ onClose }) => {
  ...
  // -- PRODUCT 1 (Center initially) --
  const p1X = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4], ["0%", "20%", "20%", "100%"]);
  const p1Z = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4], [0, 200, 200, -500]);
  const p1RotateY = useTransform(scrollYProgress, [0, 0.1, 0.2, 0.3, 0.4], [0, -15, 10, -15, -45]);
  const p1Scale = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4], [1, 1.3, 1.3, 0.5]);
  const p1Opacity = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4], [1, 1, 1, 0]);

  // -- PRODUCT 2 (Left initially) --
  const p2X = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4, 0.6, 0.7], ["-120%", "-150%", "-150%", "20%", "20%", "100%"]);
  const p2Z = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4, 0.6, 0.7], [-200, -300, -300, 200, 200, -500]);
  const p2RotateY = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4, 0.5, 0.6, 0.7], [25, 30, 30, -15, 10, -15, -45]);
  const p2Scale = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4, 0.6, 0.7], [0.7, 0.5, 0.5, 1.3, 1.3, 0.5]);
  const p2Opacity = useTransform(scrollYProgress, [0, 0.1, 0.3, 0.4, 0.6, 0.7], [0.6, 0.3, 0.3, 1, 1, 0]);

  // -- PRODUCT 3 (Right initially) --
  const p3X = useTransform(scrollYProgress, [0, 0.1, 0.6, 0.7, 0.9, 1], ["120%", "150%", "150%", "20%", "20%", "0%"]);
  const p3Z = useTransform(scrollYProgress, [0, 0.1, 0.6, 0.7, 0.9, 1], [-200, -300, -300, 200, 200, 0]);
  const p3RotateY = useTransform(scrollYProgress, [0, 0.1, 0.6, 0.7, 0.8, 0.9, 1], [-25, -30, -30, -15, 10, -15, 0]);
  const p3Scale = useTransform(scrollYProgress, [0, 0.1, 0.6, 0.7, 0.9, 1], [0.7, 0.5, 0.5, 1.3, 1.3, 1]);
  const p3Opacity = useTransform(scrollYProgress, [0, 0.1, 0.6, 0.7, 0.9, 1], [0.6, 0.3, 0.3, 1, 1, 1]);
